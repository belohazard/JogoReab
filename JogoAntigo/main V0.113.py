...
class ZooScreen:
    ...
    def show_jail_interaction_menu(self):
        # Exibe o texto da jaula ativa
        draw_text(f"Escolha um animal para a jaula de {self.active_jail.animal_type}:", font, (0, 0, 0), screen, width // 2 - 250, height // 2 - 150)

        # Lista todas as opções de animais no inventário, sem filtro
        animal_options = inventory[:]

        # Caso o inventário esteja vazio
        if not animal_options:
            draw_text("Você não tem animais no inventário!", font, (255, 0, 0), screen, width // 2 - 150, height // 2)
            pygame.display.update()
            pygame.time.delay(1000)
            self.jail_interaction_active = False  # Fecha o menu de interação
            return  # Volta para a tela principal do jogo

        selected_option = 0  # Inicializa a opção selecionada

        while self.jail_interaction_active:
            screen.fill(white)  # Limpa a tela
            draw_text(f"Escolha um animal para a jaula de {self.active_jail.animal_type}:", font, (0, 0, 0), screen, width // 2 - 250, height // 2 - 150)

            # Exibe todas as opções de animais do inventário
            ...
            pygame.display.update()  # Atualiza a tela com as opções
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    if event.key == K_DOWN:
                        selected_option = (selected_option + 1) % len(animal_options)  # Navega para baixo
                    elif event.key == K_UP:
                        selected_option = (selected_option - 1) % len(animal_options)  # Navega para cima
                    elif event.key == K_SPACE:
                        selected_animal = animal_options[selected_option]

                        # Verifica se o animal escolhido é o correto para a jaula
                        if selected_animal.type == self.active_jail.animal_type:
                            self.active_jail.captured_animals.append(selected_animal)  # Adiciona o animal à jaula
                            inventory.remove(selected_animal)  # Remove o animal do inventário
                        ...
                        # Sai do loop de interação e volta para o zoológico
                        self.jail_interaction_active = False
                        return  # Garante que o jogo retorne para a tela principal do zoológico

            # Verifica se o estado da interação foi mudado para falso e encerra o loop
            if not self.jail_interaction_active:
                break
        ...
        for animal in self.animals[:]:
            if self.player_rect.colliderect(animal.rect):
                if animal.type == "monkey":
                    ...
                    global tucan_count
                    tucan_count += 1
                inventory.append(animal)  # Adiciona o animal capturado ao inventário
